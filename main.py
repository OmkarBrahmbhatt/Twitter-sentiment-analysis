...
with st.expander('Analyze CSV'):
    upl = st.file_uploader('Upload file', type=['csv', 'xlsx'])  # Allow both CSV and Excel file uploads
    ...
    if upl:
        if upl.name.endswith('csv'):  # Handle CSV files
            df = pd.read_csv(upl)
        else:  # Handle Excel files
            df = pd.read_excel(upl)

        # Clean text and perform sentiment analysis
        df['cleaned_text'] = df['tweets'].apply(lambda x: cleantext.clean(x, clean_all=False, extra_spaces=True,
                                                                        stopwords=True, lowercase=True, numbers=True, punct=True))
        df['score'] = df['cleaned_text'].apply(score)
        ...
        wordcloud = WordCloud(stopwords=stop_words, background_color='white').generate(' '.join(df['cleaned_text']))
